...
async def probe_one(port: int, user: str, password: str, host: str = "127.0.0.1") -> str:
    try:
...
async def main() -> None:
    ...
    passwords = ["#12345678", "12345678", "123456789", "123", "password123", "vorgo_pass", "postgres", "admin", "123456", "1234", "password", ""]
        ...
        for host in hosts:
            for user in users:
                for pwd in passwords:
                    ...
                    if res == "SUCCESS":
                        print(f"  [+] SUCCESS: host='{host}', user='{user}', password='{pwd}' works!")
                        ...
                        port_open = True
                        print(f"  [-] Port {port} ({host}) is open, but invalid credentials: user='{user}', password='{pwd}'. Details: {res}")
