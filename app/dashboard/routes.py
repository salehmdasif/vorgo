...
async def login_page(
    ...
    return templates.TemplateResponse(
        "dashboard/login.html", {"request": request, "error": None}
    )
...
async def register_page(
    ...
    if user:
        return RedirectResponse(url="/dashboard", status_code=status.HTTP_303_SEE_OTHER)
    return templates.TemplateResponse(
        "dashboard/register.html", {"request": request, "error": None}
    )
...
async def register_post(
    ...
    try:
        ...
        await manager.create(user_create, request=request)

        return RedirectResponse(
            url="/login?registered=true", status_code=status.HTTP_303_SEE_OTHER
        )
...
async def dashboard_home(
    ...
    return templates.TemplateResponse(
        "dashboard/index.html", {"request": request, "user": user, "org": org}
    )


@router.get("/dashboard/setup-org", response_class=HTMLResponse)
async def setup_org_page(
    request: Request, user: User = Depends(require_dashboard_user)
):
    if user.org_id:
        return RedirectResponse(url="/dashboard")
    return templates.TemplateResponse(
        "dashboard/setup_org.html", {"request": request, "user": user, "error": None}
    )
...
async def subscribe_post(
    ...
    try:
        ...
        checkout_url = await billing_service.create_checkout_session(
            org_id=user.org_id, plan=PlanType(plan), email=user.email
        )
...
async def payoneer_checkout_mock(request: Request, org_id: UUID, plan: str, email: str):
    """Mocks the Payoneer Hosted Checkout page for simulation."""
    return templates.TemplateResponse(
        "dashboard/payoneer_checkout.html",
        {"request": request, "org_id": org_id, "plan": plan, "email": email},
    )
...
async def payoneer_checkout_complete(
    ...
    return RedirectResponse(
        url="/dashboard/billing?status=success", status_code=status.HTTP_303_SEE_OTHER
    )
...
async def view_doc(request: Request, slug: str):
    ...
    return templates.TemplateResponse(
        "docs/detail.html", {"request": request, "doc": doc, "docs_list": docs_list}
    )
...
async def view_blog_post(request: Request, slug: str):
    ...
    return templates.TemplateResponse(
        "blog/detail.html", {"request": request, "post": post}
    )
