...
def get_price_id_for_plan(plan: PlanType) -> str:
    """Retrieves the configured Stripe Price ID for a given plan type."""
    mapping = {
...
async def create_checkout_session(org_id: UUID, plan: PlanType, email: str) -> str:
    """
    Creates a Stripe Checkout Session for subscription creation.
    Passes org_id and plan in metadata for tracking in webhook responses.
    """
...
async def create_billing_portal_session(stripe_customer_id: str) -> str:
    """Creates a Stripe Billing Customer Portal session to allow plan modifications."""
...
async def handle_stripe_webhook(
    ...
    """Verifies Stripe webhook signature and dispatches updates to the database."""
    try:
        event = stripe.Webhook.construct_event(
            payload, sig_header, settings.STRIPE_WEBHOOK_SECRET
        )
    except stripe.error.SignatureVerificationError as e:
        raise ValueError("Invalid signature verification") from e
    ...
    if event_type in [
        ...
        await _process_subscription_change(db, data_object)


async def _process_subscription_change(
    db: AsyncSession, subscription: dict[str, Any]
) -> None:
    """Updates the Organization model attributes based on Stripe subscription changes."""
    ...
    # If plan is missing in metadata, resolve it back from Stripe Price ID
    if not plan_str:
    ...
    # Map Stripe status to local database enums
